...
def _extract_id(elem: Any, default_prefix: str = "elem") -> str:
    """Extrae el id del elemento buscando {XMI_NS}id o id estándar."""
    xmi_id = elem.attrib.get(f"{{{XMI_NS}}}id") or elem.attrib.get("id")
    if not xmi_id:
...
def _extract_type(elem: Any) -> str:
    """Extrae el tipo semántico buscando {XMI_NS}type o el nombre del tag."""
    xmi_type = elem.attrib.get(f"{{{XMI_NS}}}type")
    if xmi_type:
        return str(xmi_type)
...
def _extract_primitive_or_custom_type(elem: Any) -> tuple[str | None, str | None]:
    ...
    for child in elem:
        ...
        if child_local == "type":
            ...
            idref = child.attrib.get(f"{{{XMI_NS}}}idref") or child.attrib.get("idref")
            if idref:
                return str(idref), None

            child_type = child.attrib.get("type") or child.attrib.get(
                f"{{{XMI_NS}}}type"
            )
            if child_type:
...
def _parse_element_recursive(elem: Any, parent_id: str | None = None) -> XMIElement:
    ...
    for child in elem:
        ...
        if child_local == "memberEnd":
            m_ref = child.attrib.get(f"{{{XMI_NS}}}idref") or child.attrib.get("idref")
            if m_ref:
        ...
        # Extremos de conector <end role="..."/>
            ...
            role_ref = (
                ...
                or child.attrib.get(f"{{{XMI_NS}}}idref")
...
def parse_xmi_to_intermediate(content: bytes) -> XMIModelDocument:
    ...
    for child in model_elem:
        if not isinstance(child.tag, str):
            continue
        _, child_local = split_clark_qname(child.tag)
        # Ignorar extensiones propietarias o metadatos de documentación
        if child_local not in ("Extension", "documentation", ""):
            root_elements.append(_parse_element_recursive(child, parent_id=model_id))
