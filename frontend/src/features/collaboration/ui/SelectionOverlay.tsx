...
export function SelectionOverlay({ editorRef: _editorRef }: SelectionOverlayProps) {
  ...
  const [boxes, setBoxes] = useState<Map<string, { box: BoundingBox; peer: any }>>(
    new Map()
  );

  useEffect(() => {
    const remotePeers = peers.filter(
      (p) => p.selectedNode && p.clientId !== localClientId
    );
      ...
      {Array.from(boxes.values()).map(({ box, peer }) => (
        <div
          ...
          style={{
            ...
            transition:
              'transform 0.08s ease-out, width 0.08s ease-out, height 0.08s ease-out',
            pointerEvents: 'none',
          }}
        >
          <div
