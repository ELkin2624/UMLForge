...
import { YWebSocketProvider } from './websocket-provider';
...
export class CollaborationManager {
  ...
  connect(roomName: string, displayName?: string): void {
    ...
    this._offSync = this._provider.onSync((isSynced) => {
      if (!isSynced || this._syncHandled || !this._doc) return;
  ...
  private _applyDocumentAuthority(doc: Y.Doc): void {
    ...
    const localJson = localModel ? JSON.stringify(localModel) : null;

    if (remoteJson) {
      if (!localJson) {
        // No hay modelo local → adoptar el de la sala sin advertencia
        try {
        ...
        // Sala tiene modelo distinto al local → sala gana, mostramos advertencia
        try {
          const remoteModel = JSON.parse(remoteJson);
          useModelStore.getState().setModel(remoteModel);
          useModelStore.getState().setRoomNameWarning(
            'La sala ya contiene un modelo colaborativo. Tu modelo local ha sido reemplazado.'
          );
        } catch {
          console.warn('[UMLForge Collaboration] No se pudo parsear el modelo remoto.');
        }
      }
      // Si son iguales: sin advertencia, sin reemplazo
    } else if (localJson && localModel) {
      // Sala vacía + hay modelo local → publicar
      setModelJson(doc, localJson, 'umlforge-local');
    }
    // Si ambos vacíos: sin acción
  }
